...
int M2lConfig::saveToFile(QString filename)
{
    ...
    strncpy(config.deposits,m_deposits.toStdString().c_str(),30);
    strncpy(config.dtb,m_dtb.toStdString().c_str(),30);
    ...
    strncpy(config.interpolationScheme,m_interpolationScheme.toStdString().c_str(),30);
    ...
    strncpy(config.plutonForm,m_plutonForm.toStdString().c_str(),30);
    ...
    strncpy(config.nullScheme,m_nullScheme.toStdString().c_str(),30);
...
int M2lConfig::loadFromFile(QString filename)
{
    ...
    if (resp.errorCode == 0) {
        ...
        Q_EMIT quietModeChanged();
        Q_EMIT orientationDecimateChanged();
        Q_EMIT contactDecimateChanged();
        Q_EMIT intrusionModeChanged();
        Q_EMIT interpolationSpacingChanged();
        Q_EMIT misorientationChanged();
        Q_EMIT interpolationSchemeChanged();
        Q_EMIT faultDecimateChanged();
        Q_EMIT minFaultLengthChanged();
        Q_EMIT faultDipChanged();
        Q_EMIT plutonDipChanged();
        Q_EMIT plutonFormChanged();
        Q_EMIT distBufferChanged();
        Q_EMIT contactDipChanged();
        Q_EMIT contactOrientationDecimateChanged();
        Q_EMIT nullSchemeChanged();
        Q_EMIT thicknessBufferChanged();
        Q_EMIT maxThicknessAllowedChanged();
        Q_EMIT foldDecimateChanged();
        Q_EMIT fatStepChanged();
        Q_EMIT closeDipChanged();
        Q_EMIT useInterpolationsChanged();
        Q_EMIT useFatChanged();
